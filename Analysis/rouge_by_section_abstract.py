...
if not PREPARED:
    with open(BASE_DIR + "/Data/Generated_Data/Rouge_By_Section_Abstract/rouge_by_section_abstract_list.pkl", "rb") as f:
    ...
    print("\nWriting data...")
    write_dir = BASE_DIR + "/Data/Generated_Data/Rouge_By_Section_Abstract/"
    with open(write_dir + "rouge_by_section_abstract.pkl", "wb") as f:
        pickle.dump(rouge_by_section_final, f)

    with open(write_dir + "rouge_by_section_abstract_titles.csv", "wb") as f:
        writer = csv.writer(f)
        for key, val in rouge_by_section_final.iteritems():
            writer.writerow((key,))

    with open(write_dir + "rouge_by_section_abstract_values.csv", "wb") as f:
        writer = csv.writer(f)
        for key, val in rouge_by_section_final.iteritems():
...
if PREPARED:
    ...
    write_dir = BASE_DIR + "/Data/Generated_Data/Rouge_By_Section_Abstract/"
    with open(write_dir + "ROUGE_by_section_abstract_processed.csv", "rb") as f:
        reader = csv.reader(f)
        for row in reader:
    ...
    with open(write_dir + "abstract_copy_and_paste.csv", "rb") as f:
        reader=csv.reader(f)
        for row in reader:
    ...
    plt.title("ROUGE Score by Section of Paper\nin Relation to the Abstract")
    plt.savefig(GRAPH_SAVE_DIR + "rouge_by_section_abstract.png")
    plt.show()
    ...
    plt.title("Comparison of ROUGE Score by in Relation to the Abstract\nand Section and Copy/Paste Count")
    plt.savefig(GRAPH_SAVE_DIR + "rouge_copy_paste_by_section_abstract.png")
    plt.show()