...
from Dev.Models.CombinedClassifier.combined_MLP_classifier import graph, SAVE_PATH, NUM_FEATURES, ABSTRACT_DIMENSION,\
    WORD_DIMENSIONS
from operator import itemgetter
from sklearn import linear_model
from Dev.Evaluation.rouge import Rouge

SUMMARY_WRITE_LOC = BASE_DIR + "/Data/Generated_Data/Generated_Summaries/CombinedSummariser/"
...
class CombinedSummariser(Summariser):
    """
    ...
    def __init__(self):
        ...
        self.input = self.computation_graph["input"]
    ...
    def summarise(self, filename):
        ...
        with tf.Session() as sess:
            ...
            num_sents = len(paper)
            feed_feats = np.zeros((num_sents, ABSTRACT_DIMENSION + WORD_DIMENSIONS + NUM_FEATURES), dtype=np.float32)
            for i, item in enumerate(paper):
                feed_feats[i, 0:WORD_DIMENSIONS] = item[1]
                feed_feats[i, WORD_DIMENSIONS:(ABSTRACT_DIMENSION + WORD_DIMENSIONS)] = item[2]
                feed_feats[i, (ABSTRACT_DIMENSION + WORD_DIMENSIONS):] = item[3]

            # Predict how good a summary each sentence is using the computation graph
            probs = sess.run(self.prediction_probs, feed_dict={self.input: feed_feats, self.keep_prob: 1})
