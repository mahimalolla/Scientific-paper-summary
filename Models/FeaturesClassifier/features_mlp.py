...
SAVE_PATH = BASE_DIR + "/Trained_Models/" + MODEL_NAME + "/" + str(HIDDEN_LAYER_WEIGHTS) + "_units/" + MODEL_NAME \
            + "_" + str(HIDDEN_LAYER_WEIGHTS) + ".ckpt"
...
def graph():
    ...
    # ReLU
    hidden_layer = tf.nn.relu(tf.matmul(sentence_input, sent_weight) + sent_bias)
    ...
    output = tf.matmul(hidden_layer, projection_weight) + projection_bias
...
if __name__ == '__main__':
    ...
    graph_outputs = graph()
    sentence_input = graph_outputs["features_input"]
    ...
    vis_weight = graph_outputs["extra"]
