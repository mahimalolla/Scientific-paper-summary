...
FEATURE_HIDDEN_LAYER_WEIGHTS = 64
...
def graph():
    ...
    lstm_output = tf.nn.relu(tf.contrib.layers.linear(lstm_output, FINAL_LAYER_WEIGHTS))

    # ----> Preprocess the features <----

    # Pass through a ReLU
    #feats = tf.nn.relu(tf.contrib.layers.linear(features_input, WORD_DIMENSIONS))

    # Second pass
    feats2 = tf.nn.relu(tf.contrib.layers.linear(features_input, FEATURE_HIDDEN_LAYER_WEIGHTS))

    # Project the second pass to two classes
    feat_scores = tf.nn.relu(tf.contrib.layers.linear(feats2, NUM_CLASSES))

    # ----> Concatenate the feature information with the LSTM ouput <----

    # Concatenate with the LSTM output
    #full_info = tf.concat_v2([lstm_output, feats], axis=1)
    ...
    final_hidden_layer = tf.nn.dropout(lstm_output, keep_prob)

    # ----> Project the final output down to the number of classes <----

    scores = tf.nn.relu(tf.contrib.layers.linear(final_hidden_layer, NUM_CLASSES))

    # ----> Train a meta-classifier between features only and LSTM + features <----

    # Concatenate the two score measures together
    score_set = tf.concat_v2([scores, feat_scores], axis=1)

    # Project down to two
    final_scores = tf.contrib.layers.linear(score_set, NUM_CLASSES)

    # Define the loss function
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(final_scores, labels))
    opt = tf.train.AdamOptimizer(LEARNING_RATE).minimize(loss)

    # Predictions
    predictions = tf.nn.softmax(final_scores)

    # Calculate accuracy
    pred_answers = tf.argmax(final_scores, axis=1)
